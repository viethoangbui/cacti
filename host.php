	...
	if (isset_request_var('save_component_host')) {
		if (get_nfilter_request_var('snmp_version') == 3 && (get_nfilter_request_var('snmp_password') != get_nfilter_request_var('snmp_password_confirm'))) {
			...
			$host_id = api_device_save(
				...
				get_nfilter_request_var('supplier_id'),
				get_nfilter_request_var('device_type_id'),
				get_nfilter_request_var('model_id')
			);
	...
	<script type='text/javascript'>
		...
		function setAvailability() {
			...
			if ($('#snmp_version').val() == '0') {
				methods = [{
						value: '0',
						text: '<?php print __('None'); ?>'
					},
					{
						value: '3',
						text: '<?php print __('Ping'); ?>'
					}
				];
				...
				methods = [{
					...
					{
						value: '1',
						text: '<?php print __('Ping and SNMP Uptime'); ?>'
					},
					{
						value: '2',
						text: '<?php print __('SNMP Uptime'); ?>'
					},
					{
						value: '3',
						text: '<?php print __('Ping'); ?>'
					...
					{
						value: '4',
						text: '<?php print __('Ping or SNMP Uptime'); ?>'
					},
					{
						value: '5',
						text: '<?php print __('SNMP Desc'); ?>'
					},
					{
						value: '6',
						text: '<?php print __('SNMP GetNext'); ?>'
					}
				];
...
function get_device_records(&$total_rows, $rows)
{
	...
	if (get_request_var('site_id') == '0' || get_request_var('site_id') == '-1') {
		$sql_where .= ($sql_where != '' ? ' and host.site_id in (SELECT DISTINCT sites.id 
		FROM user_auth
		...
		WHERE sites.id is not null and user_auth.id = ' . $_SESSION['sess_user_id'] . ') '
			: ' WHERE   host.site_id in (SELECT DISTINCT sites.id 
		FROM user_auth
		LEFT JOIN user_auth_group_members on user_auth.id = user_auth_group_members.user_id
		LEFT JOIN user_auth_group on user_auth_group_members.group_id = user_auth_group.id
		left JOIN sites on user_auth_group.description = sites.city
	...
	$sql_query = "SELECT host.*, gl.graphs, dl.data_sources,(select name from sites where id = host.site_id) as location_name,
		CAST(IF(availability_method = 0, '0',
			...
			IF(snmp_sysUptimeInstance>0 AND host.snmp_version > 0, snmp_sysUptimeInstance/100, UNIX_TIMESTAMP()
		...
		LEFT JOIN suppliers ON host.supplier_id = suppliers.id
		LEFT JOIN device_type ON host.loaithietbi = device_type.id
		LEFT JOIN model ON host.model = model.id
		$sql_where
		$sql_order
		$sql_limit";
...
?>
	...
	$display_text = array(
		...
		'nhaccungcap' => array(
			'display' => __('Nhà cung cấp'),
			'align' => 'left',
			'sort' => 'ASC'
		),
		'loaithietbi' => array(
			'display' => __('Loại thiết bị'),
			'align' => 'left',
			'sort' => 'ASC'
		),
		'devicemodel' => array(
	...
	if (sizeof($display_text) != $display_text_size && cacti_sizeof($hosts)) { //display_text changed
		...
		foreach ($hosts as $host) {
			...
			form_selectable_cell("<span class=\"host-editor\" id=\"{$host['id']}\" style=\"padding:4px 6px;cursor:pointer;display:flex;\">	 
			...
			form_selectable_cell(filter_value($host['device_type_name'], get_request_var('filter')), $host['id']);
			form_selectable_cell(filter_value($host['model_name'], get_request_var('filter')), $host['id']);
			form_selectable_cell(filter_value($host['location_name'], get_request_var('filter')), $host['id']);
			form_selectable_cell(filter_value($host['device_type_name'], get_request_var('filter')), $host['id']);
