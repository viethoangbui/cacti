...
function get_matching_nodes()
{
	...
	if (get_nfilter_request_var('str') != '') {
		$matching = db_fetch_assoc_prepared(
...
?>
	...
	case 'get_node':
		...
		if (isset_request_var('tree_id')) {
			if (get_nfilter_request_var('tree_id') == 0 && strstr(get_nfilter_request_var('id'), 'tbranch-') !== false) {
				...
				get_nfilter_request_var('tree_id') == 'default' ||
				...
				get_nfilter_request_var('tree_id') == 0 &&
	...
	case 'preview_Customer':
		...
		if (!isempty_request_var('cl_number')) {
			...
			$graph_title = db_fetch_cell_prepared(
				'select CONCAT(cl_number, " - ", cus_name) AS graph_title from customer WHERE cl_number =  ?',
	...
	case 'preview_classMap':
		...
		if (!empty($_GET['cl_number'])) {
			...
			// 		FROM class_map
			// 		WHERE cl_number = ?',
			// 	array($_GET['cl_number'])
			// );
			// $base64 = getGraphClassMap($classMaps);
			// if (count($base64["data"]) > 0) {
			echo '<div id="graph-view-custom">';
			// echo '<img class="graphimage" src="' . $base64["total"]["image"] . '"">';
			// foreach ($base64['data'] as $value) {
			// 	if (isset($value['image'])) {
			// 		echo '<img class="graphimage" loading="lazy" src="' . $value['image'] . '"">';
			// 	}
			// }
...
<?php
	...
	case 'graph_details':
		...
		if (!isempty_request_var('graph_local_id')) {
			...
			$graph_title = db_fetch_cell_prepared(
				'select graph_templates_graph.title_cache  from graph_local
	...
	case 'testt':
		...
		$future = $runtime1->run(function () {
			...
			return executeClassMapd($path1, $path2, 'TOTAL BANDWITH', get_current_graph_start() - 2 * 86400, get_current_graph_end() - 2 * 86400);
		...
		echo $future->value();
		break;
	case 'graph_json_custom':
		$objProcessRrd = new ProcessRrd(
			$_GET['table'],
			$_GET['rrd_name_field'],
			$_GET['graph_name_field'],
			$_GET['cl_number']
		);
		...
		break;
	case 'graph_custom':
		$base64 = testGraph($_GET['rrd_file'], $_GET['rrd_file'], $_GET['rrd_name']);
		header('Content-Type: application/json; charset=utf-8');
		echo json_encode(['image' => $base64]);
		break;
}

function getGraphClassMap($classMaps)
{
	$sources = [];
	$data = [];
	$total = [];
	...
	foreach ($classMaps as $class) {
		$path = "C\:/xampp/htdocs/cacti/rra/{$class['path_graph']}";
		$sources[] = $path;

		if ($i === 0) {
			$data[$i]['image'] = executeClassMapCommand(
				[$path],
				$class['class_map_name'],
				get_current_graph_start() - 2 * 86400,
				get_current_graph_end() - 2 * 86400
			);
		} else {
			$data[$i]['image'] = executeClassMapCommand(
				[$path],
	...
	$total['image'] = executeClassMapd($path1, $path2, 'TOTAL BANDWITH', get_current_graph_start() - 2 * 86400, get_current_graph_end() - 2 * 86400);
	// $total['image'] = executeClassMapCommand(
	// 	$sources,
	// 	'TOTAL BANDWIDTH',
	// 	get_current_graph_start() - 86400,
	// 	get_current_graph_end() - 86400,
	// );
...
function executeClassMapCommand(
	$listSources,
	$title,
	$start,
	$end,
	...
	for ($i = 0; $i < count($listSources); $i++) {
		if (!empty($listSources[$i])) {
			$execs[] = "DEF:avgin_" . $i . "=" . $listSources[$i] . ":ds0:AVERAGE";
			// $execs[] = "DEF:avgout_" . $i . "=" . $listSources[$i] . ":ds0:AVERAGE";
	...
	$output = rrdtool_execute($command, false, '1', false);
