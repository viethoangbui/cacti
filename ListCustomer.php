...
function get_site_locations() {
	...
	$locations = db_fetch_assoc_prepared("SELECT DISTINCT name
		FROM sites
		WHERE status = 1
		$where
		ORDER BY name",
...
function get_Customer_records(&$total_rows, $rows) {
	...
	$sql_query = "select customer.*,sites.name as site_name
                  from customer
				  join sites on customer.site_id = sites.id
	$sql_where 
    $sql_limit";
...
function host() {
						...
						<select id='location'>
							...
							if (cacti_sizeof($locations)) {
								foreach ($locations as $l) {
									...
									if (get_request_var('location') == $l['state']) { print ' selected'; } print '>' . html_escape($l['location']) . '</option>';
								}
							}
							?>
						</select>
	...
	if (cacti_sizeof($hosts)) {
		foreach ($hosts as $host) {
			form_selectable_cell(filter_value($host['company_sn'], get_request_var('filter')), $host['company_sn']);
			form_selectable_cell(filter_value($host['user_create'], get_request_var('filter')), $host['company_sn']);
			// form_selectable_cell(filter_value($host['cl_number'], get_request_var('filter')), $host['company_sn']);
			form_selectable_cell(filter_value($host['cl_number'], get_request_var('filter'), 'graph_view.php?action=preview_Customer&cl_number=' . $host['cl_number']), $host['company_sn']);
			form_selectable_cell(filter_value($host['cus_name'], get_request_var('filter')), $host['company_sn']);
			form_selectable_cell(filter_value($host['site_name'], get_request_var('filter')), $host['company_sn']);
			form_selectable_cell(filter_value($host['StatusID'], get_request_var('filter')), $host['company_sn']);
