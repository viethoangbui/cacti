...
function form_save() {
	...
	if ((isset_request_var('save_component_data')) && (!is_error_message())) {
		...
		$input_fields = db_fetch_assoc_prepared("SELECT dtd.data_input_id, dl.host_id, dif.id, dif.input_output,
			dif.data_name, dif.regexp_match, dif.allow_nulls, dif.type_code,dl.id as data_local_id
		...
		if (cacti_sizeof($input_fields)) {
			foreach ($input_fields as $input_field) {
				if (isset_request_var('value_' . $input_field['id'])) {
					...
					SET cl_number = ?
					WHERE id = ?',
					array(get_request_var('cl_number'), $input_field['data_local_id']));
...
function ds_edit() {
	...
	if (!isempty_request_var('id')) {
		$data_local = db_fetch_row_prepared('SELECT host_id, data_template_id,cl_number
			FROM data_local
			WHERE id = ?',
			array(get_request_var('id')));
	...
	if (!empty($data['data_template_id'])) {
		...
		print "<div class='formColumnLeft'><div class='formFieldName'>CL Number</div></div>\n";
		print "<div class='formColumnRight'>";

		print "<input class='ui-state-default ui-corner-all' id='cl_number' name='cl_number' value='". html_escape($data_local['cl_number']) ."'>";
		print '</div>';
		form_hidden_box('save_component_data','1','');
...
function ds() {
	...
	if (get_request_var('orphans') == 'true') {
		...
		$data_sources = db_fetch_assoc("SELECT dtd.local_data_id,
			...
			dl.host_id, dtd.data_source_profile_id,dl.cl_number
			FROM data_local AS dl
			INNER JOIN data_template_data AS dtd
			ON dl.id = dtd.local_data_id
			INNER JOIN data_template AS dt
		...
		$data_sources = db_fetch_assoc("SELECT dtd.local_data_id, dtd.name_cache, dtd.active,
			dtd.rrd_step, dt.name AS data_template_name, dl.host_id, dtd.data_source_profile_id,dl.cl_number
			FROM data_local AS dl
			INNER JOIN data_template_data AS dtd
			ON dl.id=dtd.local_data_id
			LEFT JOIN data_template AS dt
	...
	if (cacti_sizeof($data_sources)) {
		foreach ($data_sources as $data_source) {
			...
			form_selectable_cell($data_source['cl_number'], $data_source['cl_number'], '', 'right');
