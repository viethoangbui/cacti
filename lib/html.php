			...
			if (is_array($add_text)) {
				if (cacti_sizeof($add_text)) {
					foreach($add_text as $icon) {
						...
						print "<span class='cactiFilterAdd' title='$title'><a" . (isset($icon['id']) ? " id='" . $icon['id'] . "'":'') . " class='$classo' href='$href' rel='$rel'><i class='$classi'></i></a></span>";
					}
				}
			} else {
				print '<span> </span>';
...
function html_common_header($title, $selectedTheme = '') {
	...
	print get_md5_include_js('include/js/jquery.modal.js');
}
